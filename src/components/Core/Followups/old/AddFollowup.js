...
class AddFollowup extends Component {
    ...
    saveDetails() {
        ...
        if (!hasError) {
            ...
            newObj.enquiry = '/purchases/' + this.props.subId;
    ...
    componentDidMount() {
        ...
        if (this.props.followupId) {
            axios.get(server_url + context_path + "api/" + this.props.baseUrl + "/" + this.props.followupId)
                .then(res => {
                    var formWizard = this.state.formWizard;

                    var newobj = res.data;
                    if (!newobj.nextFollowupType || newobj.nextFollowupType == null) {
                        newobj.nextFollowupType = '';
                    }
                    if (!newobj.type || newobj.type == null) {
                        newobj.type = '';
                    }
