...
class List extends Component {
    ...
    loadObjects(offset, all, callBack) {
        ...
        var url = server_url + context_path + "api/" + this.props.baseUrl + "?projection=tracking_list&page=" + (offset - 1);
        ...
        if(this.props.user.role !== 'ROLE_ADMIN') {
            url += "&createdBy=" + this.props.user.id;
        }

        if (this.state.filters.search) {
            url += "&assignedTo=" + encodeURIComponent('%' + this.state.filters.search + '%');
        }
        ...
        return (<ContentWrapper>
            ...
            <Table hover responsive>
                <thead>
                    <Sorter columns={[
                        { name: '#', sortable: false },
                        { name: 'Assigned To', sortable: true, param: 'assignedTo' },
                        { name: 'Product', sortable: false },
                        { name: 'Dispatched On', sortable: false, param: 'dispatchDate' },
                    ...
                    {this.state.objects.map((obj, i) => {
                        return (
                            <tr key={obj.id}>
                                ...
                                <td>
                                <Moment format="DD MMM YY HH:mm">{obj.dispatchDate}</Moment>
                                </td>
                                <td>
